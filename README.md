...
2. **Task** — algorithms for `int`/`double`, metrics V, K, T; experiments for N from 10⁴ to 10⁶ and four data conditions.
3. **Theoretical complexity** — O(·) for all algorithms (Insertion, Quick, Merge, Heap, Bubble, Selection, Shell, Radix, Counting and multithreaded variants).
4. **Result tables** — measurements for `int` and `double` (longtable by N).
5. **Conclusions** — performance and memory comparison, consistency with theory, algorithm choice.
6. **Code and screenshots** — repo link, console and GUI screenshots, time-vs-N charts.

Table data: `report/data_int.csv` and `report/data_double.csv`; LaTeX tables are generated by `report/gen_tables.py`.
