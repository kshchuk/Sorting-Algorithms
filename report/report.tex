...
Експерименти та вимірювання виконувалися на ноутбуці Apple MacBook Air з такими параметрами (за виводом Google Benchmark та збірки проєкту):
\begin{itemize}
\item \textbf{Модель:} Apple MacBook Air, чип Apple M4, архітектура ARM64.
\item \textbf{Процесор:} 10 логічних ядер. Номінальна частота на macOS бенчмарком не зчитується (\texttt{hw.cpufrequency} недоступний); на вимірювання часу $T$ це не впливає.
\item \textbf{Кеш L1:} Data 64\,KiB, Instruction 128\,KiB.
\item \textbf{Кеш L2:} Unified 4096\,KiB (на кожне ядро).
\item \textbf{ОС:} macOS (Darwin 25.2.0).
\item \textbf{ОЗП:} 24\,ГіБ. Для проведених розмірів масивів (до $N = 10^6$ елементів) достатня; на відносне порівняння алгоритмів за складністю та залежністю від $N$ не впливає.
\end{itemize}
Абсолютні значення часу $T$ залежать від конфігурації; на іншій машині вони можуть відрізнятися. Відносне порівняння алгоритмів за складністю та залежністю від $N$ залишається коректним.
